...
--import — <filepath>. Импортирует данные из TSV-файла. Результат работы команды import должен выводиться в консоль. Параметр <filepath> указывает путь к файлу, из которого необходимо импортировать данные.

```bash
npm run ts ./src/cli.ts -- --import ./mocks/mock-data.tsv
```
